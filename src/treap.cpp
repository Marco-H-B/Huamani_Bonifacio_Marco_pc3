...
bool buscar(Nodo* nodo, int val) {
    ...
    if (nodo->clave == val) return true;
    if (val < nodo->clave) return buscar(nodo->izq, val);
    return buscar(nodo->der, val);
}

// insertar un elemento al treap
void insertar(Nodo*& raiz, int val) {
    if (buscar(raiz, val)) return; // SPOJ dice que es un set, no repetidos
...
void eliminar(Nodo*& raiz, int val) {
    if (!buscar(raiz, val)) return; // si no esta ni me gasto
...
int kesimo(Nodo* nodo, int k) {
    if (nodo == nullptr) return -1; // por las dudas
    
    int tamIzq = obtenerTam(nodo->izq);
    
    // si k <= al tamaño de la izquierda, entonces mi k-esimo esta en la rama izquierda
    if (k <= tamIzq) {
    ...
    // si no, toca buscar a la derecha, pero le resto todo lo que deje a la izquierda y el nodo actual
